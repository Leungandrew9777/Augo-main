...
Understat publishes the new season (matches + real xG). This appends the
finished new-season matches to ``premier_league_historical_clean.csv`` so
feature_engineering.py + train_ensemble.py rebuild ELO/form features that
already include the new season.

Understat lacks shots/corners/cards/bookmaker odds, so those columns are filled
with per-column league medians (real xG still overrides the proxy). Idempotent:
skips if the season is already present.
...
CLEAN_FILE = os.path.join(APP_DIR, "premier_league_historical_clean.csv")
